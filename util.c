...
void zip_with(uint8_t (*operator)(uint8_t, uint8_t), size_t k, size_t start, size_t end,
...
void random_matrix(size_t rows, size_t columns, uint8_t a[columns][BYTES(rows)])
{
    for (size_t i = 0; i < columns; i++) {
		...
		zip_with(xor, rows, 0, rows, a[i], random, a[i]);
    }
}
...
{	
	...
	zip_with(and, k, 0, k, v, A[column], tmp);
...
void permute(size_t k, unsigned int a[k])
{
    a[0] = 0;
...
static void add_pivot(size_t dim, uint8_t a[dim][BYTES(dim)], size_t pivot_row, size_t pivot_column)
{
	for (size_t j = pivot_column + 1; j < dim; j++) {		
        if (is_pivot(pivot_row, a[j], pivot_row)) {
			zip_with(xor, dim, pivot_row, dim, a[j], a[pivot_column], a[j]);
		}	
	}
}
...
{	
            ...
            if (is_pivot(dim, a_tmp[j], i)) {
                ...
                for (size_t k = rank + 1; k < dim; k++) {
					if (is_pivot(i, a_tmp[k], i)) {			
						zip_with(xor, dim, 0, dim, a_tmp[k], a_tmp[rank], a_tmp[k]);
						zip_with(xor, dim, 0, dim, b[k], b[rank], b[k]);
					}						
                }
    ...
    for (size_t i = dim; i --> 0; ) {
        for (size_t j = i; j --> 0; ) {
            if (is_pivot(dim, a_tmp[j], i)) {
				zip_with(xor, dim, 0, dim, b[j], b[i], b[j]);
            }
        }
    }
